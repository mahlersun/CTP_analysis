...
class MopacMolecule(Molecule):
    ...
    def read_states(self,types=[]):
        ...
        if len(self.states) > 0:
            ...
            for i in self.states[1].coeff:
                print(i) 
        else:
            print "Error: Excited states not read"
    
    # Compute the %CTP on one nanowire
    def gen_ctp(self, exc): 
        if not self.states_read:
            self.read_states()
        self.exc = int(exc)
        self.ctp = 0

        rod = int(len(self.atoms)) / 2
        trans = [0 for x in range(rod*9*9)]
        surp = 0
        for i in range(len(self.states[self.exc].coeff)):
            conf = self.states[self.exc].coeff[i][0] - 1
           # print(conf)
            aoa = self.configs[conf].occ
            aob = self.configs[conf].vir
           # print(aoa)
           # print(aob)
           # dens = 0
            same = 0
            for m in range(rod):
                for n in range(m*9, m*9+9):
                    c1 = self.mos[aoa].coeff[n]
                    for k in range(m*9, m*9+9):
                        elem = c1 * self.mos[aob].coeff[k]
                        index = n*9+(k-m*9)
                        trans[index] = trans[index] + (self.states[self.exc].coeff[i][1] * elem)
                       # dens = dens + abs(self.states[self.exc].coeff[i][1] * elem)
                        if n == k:
                            same = same + self.states[self.exc].coeff[i][1] * elem
           # trans = trans + dens
            surp = surp + same
